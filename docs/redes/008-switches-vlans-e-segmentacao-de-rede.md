...
description: Entenda como um switch aprende endereços MAC, o que é MAC flooding, como VLANs segmentam uma rede física em redes lógicas, e como funciona um ataque de VLAN hopping.
---
...
O problema é que essa tabela tem **tamanho limitado**. E é exatamente isso que o ataque de **MAC flooding** explora.

### MAC Flooding

O atacante envia uma quantidade massiva de quadros com endereços MAC de origem falsos e diferentes entre si. A tabela CAM enche rapidamente, e o switch não consegue mais aprender novas associações válidas.
...
!!! tip "Por que isso importa para você, especificamente"
    MAC flooding é frequentemente usado como **passo intermediário** para permitir sniffing de tráfego (capturar senhas, sessões, dados sensíveis) em uma rede comutada, que normalmente dificultaria esse tipo de escuta.

---

# VLANs — segmentando uma rede física em redes lógicas

Uma **VLAN** (Virtual LAN) permite dividir uma única infraestrutura física de switches em múltiplas redes lógicas separadas — como se fossem redes fisicamente distintas, mesmo compartilhando o mesmo hardware.

```text
Sem VLAN:
todo dispositivo conectado ao switch está na mesma rede lógica

Com VLAN:
o mesmo switch físico pode hospedar várias redes lógicas isoladas entre si
```

Isso conecta diretamente com o que vimos em Fundamentos sobre **Defesa em Profundidade**: VLANs são uma das formas mais práticas de aplicar segmentação de rede, limitando o alcance de um possível comprometimento.

Alguns usos comuns:

- separar rede de convidados da rede corporativa;
- isolar dispositivos IoT de servidores críticos;
- segmentar diferentes departamentos de uma empresa.
...
Para o switch saber a qual VLAN cada quadro pertence quando trafega por uma porta trunk, é usada uma marcação chamada **802.1Q** — uma "etiqueta" inserida no quadro identificando sua VLAN de origem.
...
Se a porta do switch estiver configurada para negociar automaticamente o modo trunk, um atacante pode fazer seu dispositivo se passar por outro switch, convencendo a porta a virar trunk — e, com isso, ganhar acesso ao tráfego de todas as VLANs que passam por ali.

### Double Tagging

O atacante insere **duas** etiquetas 802.1Q no mesmo quadro. Alguns switches removem apenas a primeira etiqueta ao processar o quadro, encaminhando o restante — com a segunda etiqueta ainda intacta — para uma VLAN diferente da esperada, efetivamente "pulando" para outra rede lógica.
...
- monitorar o mesmo endereço MAC aparecendo em múltiplas portas em curto intervalo — possível spoofing ou sintoma de flooding;
- observar um volume anormal de endereços MAC novos aparecendo em uma única porta — possível MAC flooding em andamento;
...
> Diagnóstico: o switch, sobrecarregado, entrou em modo "fail open" e passou a se comportar como um hub, expondo tráfego que normalmente estaria isolado por porta.
...
??? question "3. Qual a diferença entre uma porta de acesso e uma porta trunk?"
...
??? question "4. Quais são as duas técnicas mais comuns de VLAN hopping?"
...
??? question "5. Por que segmentar uma rede em VLANs é uma aplicação prática de Defesa em Profundidade?"
    Porque limita o alcance de um possível comprometimento: mesmo que um atacante controle um dispositivo em uma VLAN, a segmentação dificulta o acesso direto a outras partes da rede, sem depender de uma única barreira de proteção.
...
- **VLANs** permitem segmentar uma infraestrutura física em redes lógicas isoladas;
...
| **VLAN** | Virtual LAN; rede lógica isolada dentro de uma infraestrutura física compartilhada. |
